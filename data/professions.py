...
def get_all_professions():
    # Read professions from professionsBLS2015.csv
    df_professions = pd.read_csv(".\\data\\professionsBLS2015.csv")
    professions_set = set(df_professions["label1"].dropna().str.lower())

    # Remove entries containing "."
    professions_set = {prof for prof in professions_set if "." not in prof}

    # Read job titles from employeesalarues2017.csv
    df_jobs = pd.read_csv(".\\data\\employeesalaries2017.csv")

    df_jobs = df_jobs.sort_values("2017 Annual Salary")[-1000:]
    re_express = re.compile("\d")

    df_jobs["Job Title cleaned"] = df_jobs["Job Title"].str.lower().replace(re_express, "")
    job_titles_set = set(df_jobs["Job Title cleaned"].dropna().unique().astype(str))



    # Combine both sets
    all_professions_set = professions_set.union(job_titles_set)

    return all_professions_set
