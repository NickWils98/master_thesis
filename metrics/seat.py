...
def sentence_embedding(template, word, embedding_type: EmbeddingType, tokenizer, model, gender_subspace=None):
    # CLS embedding
    if embedding_type == EmbeddingType.CLS:
        sentence = template.replace("_", word)
        inputs = tokenizer(sentence, return_tensors="pt")
        if gender_subspace is not None:
            outputs = model(**inputs, encode_only=True, remove_bias=True, bias_dir=gender_subspace)
...
def test(
    ...
    X = {
        ...
        + str(j): sentence_embedding(
            attribute_template, j, embedding_type, tokenizer, model, gender_subspace=gender_subspace
        )
        for j in XX
    }
    Y = {
    ...
    for i in range(len(female_list)):
        ...
        A = {
            "a"
            + str(j): sentence_embedding(
                target_template, j, embedding_type, tokenizer, model,gender_subspace=gender_subspace
            )
            for j in AA
        }
        B = {
...
def seat_test(attribute_template: str, target_template: str, tokenizer, model, gender_subspace=None):
    """
    SEAT test with CLS embeddings.
    """
    return test(
        attribute_template, target_template, tokenizer, model, EmbeddingType.CLS, gender_subspace=gender_subspace
    )
