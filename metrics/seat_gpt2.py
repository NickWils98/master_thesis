...
def sentence_embedding(template, word, embedding_type, tokenizer, model):
    ...
    if embedding_type == EmbeddingType.CLS:
        ...
        return token_embeddings[0,-1,:].detach().numpy()
    ...
    if embedding_type == EmbeddingType.TEMPLATES_FIRST:
        ...
        outputs = model(inputs)
        token_embeddings = outputs.last_hidden_state
        sum_embeddings = torch.sum(token_embeddings[:, 1:], dim=1)
        return sum_embeddings.detach().numpy()
    ...
    if embedding_type == EmbeddingType.NO_CONTEXT_TEMPLATES:
        ...
        outputs = model(inputs)
        token_embeddings = outputs.last_hidden_state
        return token_embeddings[:, 1].detach().numpy()
...
def construct_cossim_lookup(XY, AB):
    ...
    for i, xy_key in enumerate(XY):
        for j, ab_key in enumerate(AB):
            cossims[i, j] = cossim(XY[xy_key], AB[ab_key])
    return cossims
